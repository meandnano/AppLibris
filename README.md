...
- **Import from the browser.** An Import page takes an EPUB or FB2 off your
  laptop, shows you its cover and metadata and whether the library already
  has it, and on confirmation copies it into the library directory and
  opens its page. Needs write access to the library; without it the page
...
- **write** access to `/data`, which holds the database and the covers the
  app writes;
- **read** access to `/library`, which nothing but importing ever writes
  to. Mount it `:ro` (or export the filesystem read-only) and everything
  works except the Import page, which says so on startup and offers no nav
  link. Leave it writable, as above, to import books from the browser.
...
library already holds byte for byte is refused with a link to the book it
already is; a different file with a title you already own is offered under
a warning, since a second edition is a legitimate thing to have.
