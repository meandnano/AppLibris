...
// the only writer of a cover row — and it must run while cover_path is still
// empty, since its own re-check skips a field that is no longer missing and
// would then write the path without the provenance.
