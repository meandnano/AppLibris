...
// the user typed.
func metadataHandler(svc *service.Service, sendEnabled, enrichEnabled, importEnabled bool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := r.ParseForm(); err != nil {
			...
			if errors.As(err, &toolarge) {
				metadataError(w, r, svc, id, field, sendEnabled, enrichEnabled, importEnabled, "", "That is too long to save", fragment)
				return
			}
		...
		if err != nil {
			metadataError(w, r, svc, id, field, sendEnabled, enrichEnabled, importEnabled, submitted, service.MetadataValidationMessage(err), fragment)
			return
		}
		if detail == nil {
			http.NotFound(w, r)
...
func metadataError(w http.ResponseWriter, r *http.Request, svc *service.Service, id int64, field storage.MetadataField, sendEnabled, enrichEnabled, importEnabled bool, value, message string, fragment bool) {
	detail, err := svc.GetBook(r.Context(), id)
	if err != nil {
		slog.Error("get book failed", "id", id, "error", err)
		http.Error(w, "internal error", http.StatusInternalServerError)
	...
	page, err := makeBookDetailPage(r, svc, detail, sendEnabled, enrichEnabled, importEnabled, string(field))
	if err != nil {
