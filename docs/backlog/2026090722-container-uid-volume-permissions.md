...
The image runs as `nonroot` (uid 65532) with `WORKDIR /`, and `run`
begins with:
...
then `storage.Open` does the same for `DB_PATH`'s directory. On a NAS the
...
```
run error="create covers directory: mkdir /data/covers: permission denied"
```
...
`MkdirAll` on it is the one call that turns a legitimately read-only
...
- Whether `run` still `MkdirAll`s `LIBRARY_DIR`.
