...
- **A failure is logged at Info with the link's scheme, host and path only, and `ErrDownloadFailed` keeps a `*url.Error` as its `Op` and inner error.** The query and fragment routinely carry a signed token and the userinfo a password, `*url.Error` prints the full URL, and the log outlives both.
